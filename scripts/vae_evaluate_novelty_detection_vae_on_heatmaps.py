...
def evaluate_novelty_detection(cfg, track, condition):
    ...
    # dataset = dataset[:1500]  # we limit the number of samples to avoid unbalance
    ...
    # data_df_nominal = data_df_nominal[:1500]  # we limit the number of samples to avoid unbalance

    name = "track1-VAE-latent16-heatmaps-smoothgrad"

    vae = utils_vae.load_vae_by_name(name)

    # name = "nominal-heatmaps"
